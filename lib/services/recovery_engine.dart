...
class RecoveryEngine {
  ...
  static const int maxSameFailureAttempts = 2;
  ...
  Future<List<RecoveryAction>> diagnoseWithAlternatives(
    ...
    if ((_failureAttempts[failureKey] ?? 0) > maxSameFailureAttempts) {
      actions.add(
        RecoveryAction(
          action: 'give_up',
          params: {},
          description:
              'Action "$lastFailedAction" with value "$lastAttemptedValue" failed too many times. Giving up.',
    ...
    if (lowerScreen.contains('loading') ||
      ...
      actions.add(
        RecoveryAction(
          ...
          description:
              'App is loading or buffering. Waiting for content to load...',
          priority: 0,
        ),
      );
      return actions;
    ...
    if (lowerScreen.contains('captcha') ||
      ...
      actions.add(
        RecoveryAction(
          ...
          description:
              'A CAPTCHA or human verification step is blocking browser automation.',
          priority: 0,
        ),
      );
      return actions;
    ...
    if (lowerScreen.contains('keyboard') ||
      ...
      actions.add(
        RecoveryAction(
          ...
          description:
              'Alternative: Scroll up to reveal elements above keyboard.',
          priority: 2,
        ),
      );
      return actions;
    ...
    if (lowerScreen.contains('error') ||
      ...
      actions.add(
        RecoveryAction(
          ...
          description:
              'Error dialog detected. Attempting to dismiss with OK button.',
          priority: 1,
        ),
      );
      actions.add(
    ...
    if (lastFailedAction == 'open_app') {
      actions.add(
        RecoveryAction(
          ...
          description:
              'App may be loading. Waiting longer before next attempt.',
          priority: 1,
        ),
      );
      actions.add(
